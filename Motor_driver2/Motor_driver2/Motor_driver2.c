...
#define DIS2_on()  PORTA |= _BV(3)     //Disable Motor Driver Chip 1
#define DIS2_off()  PORTA &= ~_BV(3)   //Enable Motor Driver Chip 1
*/
...
int volatile count = 0;
	...
	ISR(TIMER3_COMPA_vect) //interrupt service routine for timer1 compare A flag
	{
		if(count == 0){
			count += 1;
			PWM3A_on();
...
int main(void)
{
	...
	OCR3A = 100;  //set PWM frequency  100*(1/8e6) =  12.5e-5 seconds               
